...
import { registerInstance, RegisterPayload } from '@/shared';
import { FORM_DATA_FIELD, GET_OTP_TYPE } from '@/shared';
        ...
        >
          ...
          >
            <p className="font-light text-[16px] leading-[26px] text-gray-700">
              ...
              <strong className="text-[#3BB1CF]">Eztek.com.vn</strong>
            </p>
          </Checkbox>
        </Form.Item>
      </div>
  ...
  const handleStepChange = async (isBack = false) => {
    ...
    if (currentStep === steps[1].id && !isBack) {
      const { address, city_id, name, phone, enterprise_name, gender_id, district_id } =
        form.getFieldsValue();
      if (!(address && city_id && name && phone && gender_id && district_id)) {
        form.validateFields([
