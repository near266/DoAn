...
import { FORM_DATA_FIELD } from '@/shared';
...
import { CHANGE_PHONE_OPTION } from '@/shared';
...
class AccountAPI {
  ...
  async getEnterpriseByIdEnterprise(id) {
    const res = await apiEnterprise.get(`/api/UserInfo/enteprise/detail?id=${id}`);
    return res.data;
  }
}
