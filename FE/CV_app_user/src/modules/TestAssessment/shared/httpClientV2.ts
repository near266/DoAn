...
import { authService, Cookies } from '@/shared';
...
api.interceptors.request.use(
  (config) => {
    const accessToken =
      Cookies.get(process.env.NEXT_PUBLIC_ACCESS_TOKEN_NAME) ||
      process.env.NEXT_PUBLIC_MEMBERSHIP_TOKEN;
    // TODO REMOVE ENV VARIABLE WHEN RELEASE
    if (accessToken) {
...
export { api as httpClientV2, cancelTokenSource };
